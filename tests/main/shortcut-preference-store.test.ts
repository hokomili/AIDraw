...
describe('main-owned shortcut preference storage', () => {
  it('uses complete defaults when missing and round-trips one strict private v1 map across restart', async () => {
    ...
    expect(JSON.parse(await readFile(value.path, 'utf8'))).toEqual({ version: 1, preferences });
  });
  ...
  it('rejects partial, unknown, conflicting, and oversized durable maps as a whole and warns once', async () => {
    ...
    delete partial.bindings['tool:pixel:fill'];
    const cases: Array<string | Buffer> = [
      JSON.stringify({ version: 1, preferences: partial }),
      JSON.stringify({ version: 1, preferences: { bindings: { ...defaults.bindings, unexpected: 'K' } } }),
      JSON.stringify({ version: 1, preferences: { bindings: { ...defaults.bindings, 'tool:pixel:fill': 'E' } } }),
      JSON.stringify({ version: 2, preferences: defaults }),
