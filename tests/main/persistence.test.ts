...
describe('.aidraw persistence', () => {
  ...
  it('opens acyclic native tilemap layer groups and rejects the equivalent two-group child cycle', async () => {
    ...
    const outer: TilemapLayer = { id: createId('map-layer'), revision: 0, name: 'Outer map group', createdAt: timestamp, updatedAt: timestamp, createdBy: HUMAN_ACTOR.id, visible: true, locked: false, opacity: 1, offsetX: 9, offsetY: -7, parallaxX: 1, parallaxY: 1, type: 'group', childIds: [] };
    const inner: TilemapLayer = { id: createId('map-layer'), revision: 0, name: 'Inner map group', createdAt: timestamp, updatedAt: timestamp, createdBy: HUMAN_ACTOR.id, parentId: outer.id, visible: true, locked: false, opacity: 1, offsetX: -3, offsetY: 5, parallaxX: 1, parallaxY: 1, type: 'group', childIds: [] };
    const objects: TilemapLayer = { id: createId('map-layer'), revision: 0, name: 'Objects', createdAt: timestamp, updatedAt: timestamp, createdBy: HUMAN_ACTOR.id, parentId: inner.id, visible: true, locked: false, opacity: 1, offsetX: 2, offsetY: 1, parallaxX: 1, parallaxY: 1, type: 'object', objects: [{ id: 'qa08-object', type: 'rectangle', x: 3, y: 4, width: 5, height: 6, properties: { role: 'spawn' } }] }; outer.childIds = [inner.id]; inner.childIds = [tile.id, objects.id]; tile.parentId = inner.id; map.layers[outer.id] = outer; map.layers[inner.id] = inner; map.layers[objects.id] = objects; map.layerIds = [outer.id];
    ...
    expect(loadedMap).toMatchObject({ orientation: 'orthogonal', layerIds: [outer.id] }); expect(loadedMap.layers[outer.id]).toMatchObject({ type: 'group', childIds: [inner.id], offsetX: 9, offsetY: -7 }); expect(loadedMap.layers[inner.id]).toMatchObject({ type: 'group', parentId: outer.id, childIds: [tile.id, objects.id], offsetX: -3, offsetY: 5 }); expect(loadedMap.layers[tile.id]).toMatchObject({ type: 'tile', parentId: inner.id, offsetX: 0, offsetY: 0 }); expect(readTileAt(loadedMap.layers[tile.id].chunks ?? {}, 1, 2)).toBe(7); expect(loadedMap.layers[objects.id]).toMatchObject({ type: 'object', parentId: inner.id, offsetX: 2, offsetY: 1, objects: [{ id: 'qa08-object', type: 'rectangle', x: 3, y: 4, width: 5, height: 6, properties: { role: 'spawn' } }] });
