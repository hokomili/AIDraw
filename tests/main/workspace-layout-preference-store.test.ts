...
describe('main-owned workspace layout preference storage', () => {
  it('uses safe defaults when missing and round-trips one strict private v1 value across restart', async () => {
    ...
    const saved = preferences({ inspectorCollapsed: true, inspectorExpandedWidth: 472 });
  ...
  it('rejects partial, unknown, invalid, and oversized files as a whole and warns once', async () => {
    const cases: Array<string | Buffer> = [
      ...
      JSON.stringify({ version: 2, preferences: DEFAULT_WORKSPACE_LAYOUT_PREFERENCES }),
      JSON.stringify({ version: 1, preferences: preferences({ inspectorExpandedWidth: MIN_INSPECTOR_EXPANDED_WIDTH - 1 }) }),
      JSON.stringify({ version: 1, preferences: preferences({ inspectorExpandedWidth: MAX_INSPECTOR_EXPANDED_WIDTH + 1 }) }),
  ...
  it('serializes rapid saves, preserves predecessor bytes on failure, and permits a later retry', async () => {
    ...
    const first = preferences({ inspectorCollapsed: true, inspectorExpandedWidth: 360 });
    const second = preferences({ inspectorCollapsed: false, inspectorExpandedWidth: 488 });
    ...
    const prior = await readFile(value.path);
    const third = preferences({ inspectorCollapsed: true, inspectorExpandedWidth: 504 });
    failNext = true;
  ...
  it('does not alter canonical state, native entries, or export output', async () => {
    ...
    await new WorkspaceLayoutPreferenceStore(value.path).save(preferences({ inspectorCollapsed: true, inspectorExpandedWidth: 480 }));
