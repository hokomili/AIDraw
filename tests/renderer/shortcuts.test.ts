...
describe('keyboard shortcut reference', () => {
  ...
  it('keeps current public and tracker truth aligned with the bounded remapping surface', async () => {
    ...
    expect(tracker).toContain('clean and live-synchronized through base `1d53be463fedd1ded023d098068256e457d477b9`');
    expect(tracker).toContain('The current bounded MAP-07/MAP-13 source/headless candidate is one reviewable exact-ID image-collection source replacement over that base');
    expect(tracker).toContain('retaining the local ID/GIDs/metadata and changing only the source reference plus derived nominal envelope');
