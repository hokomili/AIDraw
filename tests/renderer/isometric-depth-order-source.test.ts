...
describe('tilemap traversal source wiring', () => {
  it('preserves sparse-cell order while bounding editor and headless payload decode', async () => {
    ...
    expect(interactive).toContain('layerOffsetX: translation.x,');
    expect(interactive).toContain('layerOffsetY: translation.y,');
    ...
    expect(interactive).toContain('const overlayRegionFilter = overlayGeometry && activeRasterViewportRegion');
    expect(interactive).toContain('const selectionRegionFilter = overlayGeometry && activeRasterViewportRegion');
    ...
    expect(headless).toContain('}, layerRegion);');
