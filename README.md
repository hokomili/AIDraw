...
Clipboard copy writes standard SVG or fragment JSON text, a PNG fallback, and a private HTML envelope for editable AIDraw fragments. Current envelopes are versioned and SHA-256-bound, accept at most 2 MiB of canonical fragment JSON inside at most 16 MiB of HTML, and remain backward-compatible with the exact former `data-aidraw` attribute. A malformed envelope that claims AIDraw ownership fails explicitly instead of falling back to a possibly different bitmap; HTML without that marker may still use a PNG whose native and decoded dimensions agree and fit the existing 8,192-side/16MP and 1,500,000-byte editable-asset limits. Copy rendering and pixel-paste quantization use the supervised raster utility lane; Electron image objects only cross the OS clipboard boundary. This is an application-private HTML convention, not a new `.aidraw` schema or custom OS MIME, and source/headless tests do not establish cross-application or packaged clipboard compatibility.
...
Keyboard users can open a searchable current-workspace shortcut guide with F1 or ?, or through the named top-bar keyboard button. The vertical tool rail has one tab stop and supports Up/Down/Home/End focus movement before Enter or Space activates a tool; each tool exposes its direct key, and Tab reveals a Skip to canvas control for either drawing mode. The open-document tablist likewise has one active tab stop: Left/Right wraps across and activates documents, Home/End selects an edge, and Tab reaches a separate close button for the active document. A successful close returns focus to the replacement active tab. All documents opens on the active entry and uses Up/Down/Home/End across document and batch commands; Export focuses its first form/format control. Escape closes either popup and returns its trigger. Modal dialogs suppress background New, tool, editing, clipboard, and history shortcuts. This documents the current fixed commands; shortcut remapping, direct keyboard drawing alternatives, complete menu/focus coverage, typeahead, and packaged screen-reader/key-layout acceptance remain open.
