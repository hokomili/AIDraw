...
Keyboard users can open a searchable current-workspace shortcut guide with F1 or ?, or through the named top-bar keyboard button. The vertical tool rail has one tab stop and supports Up/Down/Home/End focus movement before Enter or Space activates a tool; each tool exposes its direct key, and Tab reveals a Skip to canvas control for either drawing mode. Modal dialogs suppress background New, tool, editing, clipboard, and history shortcuts. This documents the current fixed commands; shortcut remapping, direct keyboard drawing alternatives, complete focus order, and packaged screen-reader/key-layout acceptance remain open.
