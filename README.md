...
Illustration lasso selection evaluates canonical object geometry when a gesture completes instead of selecting every object whose rotated bounds overlap. Filled shapes and editable paths use their transformed fill silhouette (including even-odd holes), vector strokes use the same pressure outline as rendering, text and images use their transformed object frame, and stroke-only shapes/paths use their geometric centerline. Replace/add/subtract/intersect and Ctrl/Cmd containment behavior remain unchanged; unsupported groups or malformed paths retain the transformed-bounds fallback so they remain selectable. This is zoom-independent core-geometry selection, not an outer stroke-width, mask, blur, shadow/effect, or packaged pointer-acceptance promise.
