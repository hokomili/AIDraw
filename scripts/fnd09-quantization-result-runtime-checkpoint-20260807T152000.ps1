...
$repo = 'E:\AIDraw'
$outName = 'out-fnd09-quantization-result-20260807T145500'
$packageRoot = 'E:\AIDraw\out-fnd09-quantization-result-20260807T145500'
$exe = 'E:\AIDraw\out-fnd09-quantization-result-20260807T145500\AIDraw-win32-x64\AIDraw.exe'
$asar = 'E:\AIDraw\out-fnd09-quantization-result-20260807T145500\AIDraw-win32-x64\resources\app.asar'
$profile = 'E:\AIDraw\test-results\retained\aidraw-e2e-fnd09-quantization-result-native-20260807T152000'
$output = 'E:\AIDraw\test-results\playwright-fnd09-quantization-result-native-20260807T152000'
$connectionPath = Join-Path $profile 'mcp-connection.json'
$probePath = Join-Path $profile 'fnd09-quantization-result-probe.json'
$evidencePath = Join-Path $profile 'fnd09-quantization-result-evidence.json'
...
$sentinels = @(
  ...
  (Join-Path $profile 'fnd09-quantization-result-forbidden-network.json')
)
$packageListProfile = 'E:\AIDraw\test-results\retained\aidraw-e2e-fnd09-quantization-result-20260807T145500'
$packageListOutput = 'E:\AIDraw\test-results\playwright-fnd09-quantization-result-20260807T145500'
$agt14Root = 'E:\AIDraw\test-results\retained\aidraw-agt14-opencode-discovery-20260806T040500'
$packageGuard = 'E:\AIDraw\scripts\fnd09-quantization-result-package-checkpoint-20260807T145500.ps1'
...
$scenarioTitle = 'FND-09-QUANTIZATION-RESULT exact package rejects invalid child output and recovers queued quantization work'

$expectedSourceFiles = 277
$expectedSourceBytes = [long]3432201
$expectedSourceSha256 = 'A642C88332E6E89F4EAD65DBA1CE3B347DBD51FF27B537A692486C19B2CE77B9'
$expectedExeBytes = [long]225614336
$expectedExeSha256 = '1CF937B4CCD8C23128975691BFD1E436426C713811CA6DEF57B5D2CE8BE5DCCE'
$expectedAsarBytes = [long]47606959
$expectedAsarSha256 = 'CABAC58E83F5A46692A49C921FE05480124BCA13DD46DEB7A3C8B0919AC2C815'
$expectedObserverSha256 = '1DC2D0F7C98F5B5DD7BE018F037A859B2EB6B61DF4559AAF2C3953C1644BE684'
...
$failedObservationProfile = 'E:\AIDraw\test-results\retained\aidraw-e2e-fnd09-observation-codec-native-20260807T134502'
$failedObservationOutput = 'E:\AIDraw\test-results\playwright-fnd09-observation-codec-native-20260807T134502'
$passedObservationProfile = 'E:\AIDraw\test-results\retained\aidraw-e2e-fnd09-observation-codec-colorfix-20260807T140828'
$passedObservationOutput = 'E:\AIDraw\test-results\playwright-fnd09-observation-codec-colorfix-20260807T140828'
$immutableTrees = @(
  [pscustomobject]@{ Path = $failedObservationProfile; Directories = 18; Files = 26; Bytes = [long]90015; Hash = 'D88AC6EB1497F51FD5C820C9B8E7790D1A57280671BE006236429AC8326D9E90' },
  [pscustomobject]@{ Path = $failedObservationOutput; Directories = 0; Files = 1; Bytes = [long]45; Hash = 'E9C8773F9C896351AEBEBCF13A1A6958D8D3AC1A65EF1053D885244EBAA3B22C' },
  [pscustomobject]@{ Path = $passedObservationProfile; Directories = 18; Files = 26; Bytes = [long]90020; Hash = '0CD6D98398D0B913EE211721B6EAEE843FF47B88726C50F7283FB32D4563D167' },
  [pscustomobject]@{ Path = $passedObservationOutput; Directories = 0; Files = 1; Bytes = [long]45; Hash = 'E9C8773F9C896351AEBEBCF13A1A6958D8D3AC1A65EF1053D885244EBAA3B22C' }
)
...
$fixedFiles = @(
  ...
  [pscustomobject]@{ Path = (Join-Path $repo 'tests\e2e\utility-containment.spec.ts'); Bytes = [long]50496; Hash = $expectedObserverSha256 },
  [pscustomobject]@{ Path = (Join-Path $repo 'scripts\packaged-utility-quantization-result.mjs'); Bytes = [long]7531; Hash = 'F4B15B6497E420BBE35194CEF84AD0FDEEC621892CD3DB7D8BFFFB68BCA1E3B0' },
  [pscustomobject]@{ Path = (Join-Path $repo 'scripts\packaged-utility-quantization-result.d.mts'); Bytes = [long]689; Hash = '23DF35F64833CF4BCE13D9FD8A7D685382DA91DAC4ACD209C01AE43463CDCD61' },
  [pscustomobject]@{ Path = (Join-Path $repo 'scripts\packaged-utility-observation-codec.mjs'); Bytes = [long]7196; Hash = '41BC7CE43D57BE3E330DD7B0D9E212C6DD9A7E26BB0455717D8F88B24D792766' },
  [pscustomobject]@{ Path = (Join-Path $repo 'scripts\verify-package.mjs'); Bytes = [long]6455; Hash = '983616383FDF554573E616E17328A5E1A42DF89826C330C3169905054EF184F9' },
  ...
  [pscustomobject]@{ Path = $packageGuard; Bytes = [long]23250; Hash = 'CE2B760EFB4F38D122267AC7A7115B2027325B845E8F0E79E5DEFAE8F1477FDC' },
...
function Assert-ExactPlaywrightListOutput([string]$Text) {
  ...
  $declarationPattern = "(?m)^\s*(?:tests[\\/]e2e[\\/])?utility-containment\.spec\.ts:715:5\s+›\s+$escapedTitle\s*$"
  $declarations = [regex]::Matches($Text, $declarationPattern)
  if ($declarations.Count -ne 1) { throw 'List output must contain one exact quantization-result declaration at utility-containment.spec.ts:715:5.' }
  if ([regex]::Matches($Text, $escapedTitle).Count -ne 1) { throw 'List output repeats or omits the exact quantization-result title.' }
...
function Assert-ExactPlaywrightRunOutput([string]$Text) {
  $escapedTitle = [regex]::Escape($scenarioTitle)
  $selectionPattern = "(?:tests[\\/]e2e[\\/])?utility-containment\.spec\.ts:715:5\s+›\s+$escapedTitle"
  if ([regex]::Matches($Text, $selectionPattern).Count -ne 1) { throw 'Runtime output must contain exactly one exact quantization-result test selection.' }
  if ([regex]::Matches($Text, $escapedTitle).Count -ne 1) { throw 'Runtime output repeats or omits the exact quantization-result title.' }
...
function Invoke-ParserSelfTest {
  $accepted = @(
    "utility-containment.spec.ts:715:5 › $scenarioTitle`nTotal: 1 test in 1 file",
    "tests/e2e/utility-containment.spec.ts:715:5 › $scenarioTitle`nTotal: 1 test in 1 file",
    "tests\e2e\utility-containment.spec.ts:715:5 › $scenarioTitle`nTotal: 1 test in 1 file"
  )
  foreach ($fixture in $accepted) { [void](Assert-ExactPlaywrightListOutput $fixture) }
  $rejected = @(
    "other.spec.ts:715:5 › $scenarioTitle`nTotal: 1 test in 1 file",
    "utility-containment.spec.ts:716:5 › $scenarioTitle`nTotal: 1 test in 1 file",
    "nested/utility-containment.spec.ts:715:5 › $scenarioTitle`nTotal: 1 test in 1 file",
    "utility-containment.spec.ts:715:5 › $scenarioTitle`nother.spec.ts:1:1 › another test`nTotal: 2 tests in 1 file",
    "utility-containment.spec.ts:715:5 › $scenarioTitle`nTotal: 2 tests in 1 file"
  )
  ...
  [void](Assert-ExactPlaywrightRunOutput "Running 1 test using 1 worker`n  ok 1 utility-containment.spec.ts:715:5 › $scenarioTitle (4.1s)`n`n  1 passed (4.8s)")
  $runRejected = $false
  try { [void](Assert-ExactPlaywrightRunOutput "utility-containment.spec.ts:715:5 › $scenarioTitle`n1 failed (4.8s)") }
...
function Invoke-NativeCapture([scriptblock]$Command) {
  ...
  try {
    # Windows PowerShell presents native stderr as ErrorRecord objects. Capture
    # those records and use the native exit code as the authoritative result.
...
function Assert-ProcessBoundary($processes, [string]$phase) {
  ...
  if ($runOwned.Count -ne 0) {
    ...
    throw "Exact quantization-result package process exists during $phase (PIDs $survivors); retain all run evidence and do not control it."
  }
...
if ($SelfTestOnly) {
  [pscustomobject]@{
    Result = 'FND-09 quantization-result runtime guard parser/no-force/color/stderr self-test passed.'
    PowerShellVersion = $PSVersionTable.PSVersion.ToString()
    Ast = $guardAudit
    Environment = $environmentAudit
    ExactListLine = 'utility-containment.spec.ts:715:5'
...
if (!(Test-Path -LiteralPath $packageRoot -PathType Container)) { throw 'The immutable quantization-result package root is absent.' }
...
$env:AIDRAW_E2E_FND09_QUANTIZATION_RESULT_PROFILE = $profile
$env:AIDRAW_E2E_FND09_QUANTIZATION_RESULT_EXE_SHA256 = $expectedExeSha256
$env:AIDRAW_E2E_FND09_QUANTIZATION_RESULT_ASAR_SHA256 = $expectedAsarSha256
...
if ($verifyResult.ExitCode -ne 0) { throw "Static verification rejected the immutable quantization-result package.`n$($verifyResult.Text)" }
...
if ($verify.packagedUtilityQuantizationResult.isolatedMainHook -ne $true -or $verify.packagedUtilityQuantizationResult.realUtilityFaultAfterQuantization -ne $true -or $verify.packagedUtilityQuantizationResult.requestDerivedResultGate -ne $true -or $verify.packagedUtilityQuantizationResult.queuedFreshWorkerRecovery -ne $true -or $verify.packagedUtilityQuantizationResult.preloadPrivate -ne $true -or $verify.packagedUtilityQuantizationResult.rendererPrivate -ne $true) { throw 'Static quantization-result package markers failed.' }
...
$observerAuditResult = Invoke-NativeCapture { & $node '.\scripts\packaged-utility-quantization-result.mjs' '.\tests\e2e\utility-containment.spec.ts' }
if ($observerAuditResult.ExitCode -ne 0) { throw "External quantization-result observer no-force audit failed.`n$($observerAuditResult.Text)" }
$observerAudit = ConvertFrom-CapturedJson $observerAuditResult 'External quantization-result observer audit'
if ($observerAudit.observerSha256 -ne $expectedObserverSha256 -or $observerAudit.contract.onePackageLaunch -ne $true -or $observerAudit.contract.playwrightWatchdogDisabled -ne $true -or $observerAudit.contract.gracefulTimeoutRejectsWithoutControl -ne $true -or $observerAudit.contract.toolDiscoveryPrivate -ne $true -or $observerAudit.contract.invalidPayloadPrivate -ne $true) { throw 'External observer contract returned contradictory no-force/privacy evidence.' }

$listResult = Invoke-NativeCapture { & $node '.\scripts\npm-node24.mjs' run test:e2e:only -- --list --grep=FND-09-QUANTIZATION-RESULT --output=$output --workers=1 --reporter=list }
...
# Exactly one test invocation is allowed. There is no retry and this controller
# never removes run evidence or signals any subject, helper, or protected process.
$runResult = Invoke-NativeCapture { & $node '.\scripts\npm-node24.mjs' run test:e2e:only -- --grep=FND-09-QUANTIZATION-RESULT --output=$output --workers=1 --reporter=list }
...
if ($probe.version -ne 1 -or $probe.scenario -ne 'FND-09 packaged quantization result containment' -or $probe.result -ne 'passed') { throw 'Quantization-result probe identity/result failed.' }
...
if ($probe.overBudget.error.message -ne 'Raster utility quantization result exceeds its 1-pixel output budget.' -or $probe.overBudget.resultReturnedToCaller -ne $false -or $probe.overBudget.payloadRetained -ne $false -or $probe.overBudget.requestPixelBudget -ne 1 -or $probe.overBudget.returnedChanges -ne 2 -or $probe.overBudget.countGateBeforeDuplicateAllocation -ne $true -or [long]$probe.overBudget.workerPid -le 0) { throw 'Over-budget count-before-allocation rejection/privacy evidence failed.' }
if ($probe.overBudget.queuedRecovery.workerReplaced -ne $true -or $probe.overBudget.queuedRecovery.queued -ne $true -or $probe.overBudget.queuedRecovery.changes -ne 1 -or $probe.overBudget.queuedRecovery.sha256 -cnotmatch '^[A-F0-9]{64}$' -or [long]$probe.overBudget.queuedRecovery.workerPid -le 0) { throw 'First queued fresh-worker recovery evidence failed.' }
if ($probe.contradictory.error.message -ne 'Raster utility returned a malformed quantization result.' -or $probe.contradictory.resultReturnedToCaller -ne $false -or $probe.contradictory.payloadRetained -ne $false -or $probe.contradictory.contradiction -ne 'x=1 lies outside the originating 1x1 request' -or [long]$probe.contradictory.workerPid -le 0) { throw 'Contradictory-geometry rejection/privacy evidence failed.' }
if ($probe.contradictory.queuedRecovery.workerReplaced -ne $true -or $probe.contradictory.queuedRecovery.queued -ne $true -or $probe.contradictory.queuedRecovery.changes -ne 1 -or $probe.contradictory.queuedRecovery.sha256 -cnotmatch '^[A-F0-9]{64}$' -or [long]$probe.contradictory.queuedRecovery.workerPid -le 0) { throw 'Second queued fresh-worker recovery evidence failed.' }
...
if ($probe.overBudget.workerPid -eq $probe.overBudget.queuedRecovery.workerPid -or $probe.contradictory.workerPid -ne $probe.overBudget.queuedRecovery.workerPid -or $probe.contradictory.workerPid -eq $probe.contradictory.queuedRecovery.workerPid) { throw 'Worker replacement sequence is contradictory.' }
if (@(@($probe.overBudget.workerPid, $probe.contradictory.workerPid, $probe.contradictory.queuedRecovery.workerPid) | Select-Object -Unique).Count -ne 3) { throw 'The exact invalid/recovery worker sequence is not distinct.' }
if ($probe.overBudget.queuedRecovery.sha256 -ne $probe.contradictory.queuedRecovery.sha256) { throw 'Recovered valid quantization hashes disagree.' }
...
if ($evidence.actor.name -ne 'FND-09 quantization result observer' -or $evidence.actor.color -ne '#6b4f9c') { throw 'Authenticated actor evidence failed.' }
if ($evidence.quantizationResult.result -ne 'passed' -or $evidence.quantizationResult.canonical.before.sha256 -ne $probe.canonical.before.sha256 -or $evidence.quantizationResult.overBudget.workerPid -ne $probe.overBudget.workerPid -or $evidence.quantizationResult.overBudget.queuedRecovery.workerPid -ne $probe.overBudget.queuedRecovery.workerPid -or $evidence.quantizationResult.contradictory.workerPid -ne $probe.contradictory.workerPid -or $evidence.quantizationResult.contradictory.queuedRecovery.workerPid -ne $probe.contradictory.queuedRecovery.workerPid) { throw 'Sanitized evidence does not embed the exact probe result.' }
...
if ($testExit -ne 0) { throw "Exact quantization-result test failed with exit code $testExit; retain every artifact and do not control any process.`n$runText" }
$runSelection = Assert-ExactPlaywrightRunOutput $runText

[pscustomobject]@{
  Result = 'FND-09 quantization-result exact packaged runtime acceptance passed.'
  ...
  Profile = $profile
  Output = $output
  OverBudgetWorkerPid = $probe.overBudget.workerPid
  FirstRecoveryWorkerPid = $probe.overBudget.queuedRecovery.workerPid
  ContradictoryWorkerPid = $probe.contradictory.workerPid
  SecondRecoveryWorkerPid = $probe.contradictory.queuedRecovery.workerPid
