...
function requireText(source, fragments, label) {
  for (const fragment of fragments) if (!source.includes(fragment)) throw new Error(`QA-08 ${label} is missing: ${fragment}`);
}

function forbidText(source, patterns, label) {
  for (const pattern of patterns) if (pattern.test(source)) throw new Error(`QA-08 ${label} contains forbidden behavior: ${pattern}`);
}

export async function assertQa08TiledCellBudgetGate(root = process.cwd()) {
  const controller = await readFile(resolve(root, 'scripts/qa08-tiled-cell-budget-gate.mjs'), 'utf8');
  const config = await readFile(resolve(root, 'scripts/vitest.qa08-tiled-cell-budget.config.mjs'), 'utf8');
  const acceptance = await readFile(resolve(root, 'tests/opt-in/qa08-tiled-cell-budget.acceptance.ts'), 'utf8');
  ...
  forbidText(controller, [/\bexec(?:File|Sync)?\s*\(/, /\bspawn\s*\(/, /process\.kill\s*\(/, /child\.kill\s*\(/, /taskkill/i, /Stop-Process/i, /\brm(?:Sync)?\s*\(/], 'controller');

  requireText(config, [
    "include: ['tests/opt-in/qa08-tiled-cell-budget.acceptance.ts']",
  ...
  if (!defaultConfig.includes("include: ['tests/**/*.test.ts']")) throw new Error('QA-08 cannot prove isolation from the default fast-suite pattern.');
  if (defaultConfig.includes('qa08-tiled-cell-budget.acceptance.ts')) throw new Error('QA-08 high-memory acceptance leaked into the default fast suite.');
  return {
    optInOnly: true,
...
async function sourceFiles(root, directory) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isSymbolicLink()) throw new Error(`QA-08 source manifest refuses symlink ${path}.`);
...
export async function qa08TiledCellBudgetSourceManifest(root = process.cwd()) {
  const paths = [
...
async function main() {
  if (process.argv[2] === '--check') { process.stdout.write(`${JSON.stringify(await assertQa08TiledCellBudgetGate())}\n`); return; }
  if (process.argv[2] === '--manifest') { process.stdout.write(`${JSON.stringify(await qa08TiledCellBudgetSourceManifest())}\n`); return; }
