...
function sliceBetween(source, startMarker, endMarker, label) {
  ...
  if (start < 0) throw new Error(`Export-result observer source is missing ${label}.`);
  if (source.indexOf(startMarker, start + startMarker.length) >= 0) throw new Error(`Export-result observer source repeats ${label}.`);
  const end = source.indexOf(endMarker, start + startMarker.length);
  if (end < 0) throw new Error(`Export-result observer source cannot bound ${label}.`);
  return source.slice(start, end);
}

function assertForceControlAbsent(label, source) {
  const matched = FORCE_CONTROL_PATTERNS.find(({ pattern }) => pattern.test(source));
  if (matched) throw new Error(`FND-09 export-result ${label} regained forbidden force-control behavior (${matched.label}).`);
}

export function assertPackagedUtilityExportResultSources({ mainSource, workerSource, preloadSource, rendererSource }) {
  const missingMain = MAIN_MARKERS.filter((marker) => !mainSource.includes(marker));
  if (missingMain.length) throw new Error(`Packaged main process is missing FND-09 export-result markers: ${missingMain.join(', ')}.`);
  const missingWorker = WORKER_MARKERS.filter((marker) => !workerSource.includes(marker));
  if (missingWorker.length) throw new Error(`Packaged utility worker is missing FND-09 export-result markers: ${missingWorker.join(', ')}.`);
  for (const [label, source] of [['preload', preloadSource], ['renderer', rendererSource]]) {
    ...
    if (leaked.length) throw new Error(`Packaged ${label} exposes private FND-09 export-result markers: ${leaked.join(', ')}.`);
  }
  return {
...
export function assertExportResultObserverNoForceSource(observerSource) {
  ...
  const title = "const exportResultScenarioName = 'FND-09-EXPORT-RESULT exact package rejects invalid artifact envelopes and recovers queued export work';";
  if (observerSource.split(title).length !== 2) throw new Error('Observer source must contain exactly one FND-09 export-result title.');
  const scenario = sliceBetween(
    observerSource,
    'test(exportResultScenarioName, async () => {',
    '// FND-09-EXPORT-RESULT observer end.',
    'export-result scenario',
  );
  if ((scenario.match(/test\.setTimeout\(0\);/g) ?? []).length !== 1) {
    throw new Error('Export-result scenario regained forbidden force-control behavior (Playwright timeout termination is not disabled exactly once).');
  }
  if (/test\.setTimeout\((?!0\))/g.test(scenario)) throw new Error('Export-result scenario regained a timeout-driven worker-termination path.');
  for (const marker of [
    'AIDRAW_E2E_FND09_EXPORT_RESULT_EXE_SHA256',
    'AIDRAW_E2E_FND09_EXPORT_RESULT_ASAR_SHA256',
  ...
  if ((scenario.match(/const child = spawn\(packagedExecutable,/g) ?? []).length !== 1) {
    throw new Error('Export-result scenario must retain exactly one package launch.');
  }
...
async function runObserverAudit(observerPathArgument) {
  ...
  return {
    ...
    contract: assertExportResultObserverNoForceSource(observerSource),
  };
}
