...
$profile = 'E:\AIDraw\test-results\retained\aidraw-e2e-fnd09-quantization-result-20260807T145500'
$output = 'E:\AIDraw\test-results\playwright-fnd09-quantization-result-20260807T145500'
...
$fixedFiles = @(
  ...
  [pscustomobject]@{ Path = (Join-Path $repo 'tests\e2e\utility-containment.spec.ts'); Bytes = [long]50496; Hash = '1DC2D0F7C98F5B5DD7BE018F037A859B2EB6B61DF4559AAF2C3953C1644BE684' },
...
function Invoke-ParserSelfTest {
  ...
  foreach ($fixture in $rejected) {
    ...
    if (!$rejectedAsExpected) { throw 'The quantization-result list parser accepted an ambiguous regression fixture.' }
  }
}
...
function Assert-ProcessBoundary($processes, [string]$phase) {
  if (@($processes | Where-Object { $_.ExecutablePath -eq $exe }).Count -ne 0) { throw "The new quantization-result package is running during $phase; stop without process control." }
...
if ($SelfTestOnly) {
  [pscustomobject]@{
    Result = 'FND-09 quantization-result package/list guard parser, no-force, color and stderr self-test passed.'
    Ast = $guardAudit
...
if ($verify.verified -ne $true -or $verify.packagedUtilityQuantizationResult.isolatedMainHook -ne $true -or $verify.packagedUtilityQuantizationResult.realUtilityFaultAfterQuantization -ne $true -or $verify.packagedUtilityQuantizationResult.requestDerivedResultGate -ne $true -or $verify.packagedUtilityQuantizationResult.queuedFreshWorkerRecovery -ne $true -or $verify.packagedUtilityQuantizationResult.preloadPrivate -ne $true -or $verify.packagedUtilityQuantizationResult.rendererPrivate -ne $true) {
  throw 'Static quantization-result package markers failed.'
}
...
$observerResult = Invoke-NativeCapture { & $node '.\scripts\packaged-utility-quantization-result.mjs' '.\tests\e2e\utility-containment.spec.ts' }
if ($observerResult.ExitCode -ne 0) { throw "External quantization-result observer audit failed.`n$($observerResult.Text)" }
$observer = ConvertFrom-CapturedJson $observerResult 'External quantization-result observer audit'
if ($observer.observerSha256 -ne '1DC2D0F7C98F5B5DD7BE018F037A859B2EB6B61DF4559AAF2C3953C1644BE684' -or $observer.contract.onePackageLaunch -ne $true -or $observer.contract.playwrightWatchdogDisabled -ne $true -or $observer.contract.gracefulTimeoutRejectsWithoutControl -ne $true -or $observer.contract.toolDiscoveryPrivate -ne $true -or $observer.contract.invalidPayloadPrivate -ne $true) {
  throw 'External observer contract returned contradictory no-force/privacy evidence.'
}
...
$env:AIDRAW_E2E_FND09_QUANTIZATION_RESULT_EXE_SHA256 = $exeSha256
$env:AIDRAW_E2E_FND09_QUANTIZATION_RESULT_ASAR_SHA256 = $asarSha256
...
[pscustomobject]@{
  Result = 'FND-09 quantization-result package/static/list checkpoint passed without native application launch.'
  ...
  Selector = 'FND-09-QUANTIZATION-RESULT'
