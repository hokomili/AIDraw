...
$profile = 'E:\AIDraw\test-results\retained\aidraw-e2e-fnd09-import-result-20260807T170500'
$output = 'E:\AIDraw\test-results\playwright-fnd09-import-result-20260807T170500'
...
$observerChecker = Join-Path $repo 'scripts\packaged-utility-import-result.mjs'
...
$fixedFiles = @(
  ...
  [pscustomobject]@{ Path = $observerChecker; Bytes = [long]7534; Hash = '5A81D164840747278C0A462E8C8E118D7A5B31731393F2BA33A0DF722EF92546' },
...
function Invoke-ParserSelfTest {
  ...
  foreach ($fixture in $rejected) {
    ...
    if (!$rejectedAsExpected) { throw 'The import-result list parser accepted an ambiguous regression fixture.' }
  }
}
...
function Assert-ProcessBoundary($processes, [string]$phase) {
  if (@($processes | Where-Object { $_.ExecutablePath -eq $exe }).Count -ne 0) { throw "The new import-result package is running during $phase; stop without process control." }
...
if ($SelfTestOnly) {
  [pscustomobject]@{
    Result = 'FND-09 import-result package/list guard parser, no-force, color and stderr self-test passed.'
...
if ($verify.verified -ne $true -or $verify.packagedUtilityImportResult.isolatedMainHook -ne $true -or $verify.packagedUtilityImportResult.realUtilityFaultAfterImport -ne $true -or $verify.packagedUtilityImportResult.canonicalMigrationAndWarningGate -ne $true -or $verify.packagedUtilityImportResult.queuedFreshWorkerRecovery -ne $true -or $verify.packagedUtilityImportResult.preloadPrivate -ne $true -or $verify.packagedUtilityImportResult.rendererPrivate -ne $true) {
  throw 'Static import-result package markers failed.'
}
...
$observerResult = Invoke-NativeCapture { & $node '.\scripts\packaged-utility-import-result.mjs' '.\tests\e2e\utility-containment.spec.ts' }
if ($observerResult.ExitCode -ne 0) { throw "External import-result observer audit failed.`n$($observerResult.Text)" }
$observer = ConvertFrom-CapturedJson $observerResult 'External import-result observer audit'
if ($observer.observerSha256 -ne $expectedObserverSha256 -or $observer.contract.onePackageLaunch -ne $true -or $observer.contract.playwrightWatchdogDisabled -ne $true -or $observer.contract.gracefulTimeoutRejectsWithoutControl -ne $true -or $observer.contract.toolDiscoveryPrivate -ne $true -or $observer.contract.invalidPayloadPrivate -ne $true -or $observer.contract.oneTestOwnedImportInput -ne $true -or $observer.contract.zeroOutputTargets -ne $true) {
...
$env:AIDRAW_E2E_FND09_IMPORT_RESULT_EXE_SHA256 = $exeSha256
$env:AIDRAW_E2E_FND09_IMPORT_RESULT_ASAR_SHA256 = $asarSha256
...
[pscustomobject]@{
  Result = 'FND-09 import-result package/static/list checkpoint passed without native application launch.'
