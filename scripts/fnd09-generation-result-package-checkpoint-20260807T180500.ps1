...
$profile = 'E:\AIDraw\test-results\retained\aidraw-e2e-fnd09-generation-result-20260807T180500'
$output = 'E:\AIDraw\test-results\playwright-fnd09-generation-result-20260807T180500'
...
$observerChecker = Join-Path $repo 'scripts\packaged-utility-generation-result.mjs'
...
$fixedFiles = @(
  ...
  [pscustomobject]@{ Path = $observerChecker; Bytes = [long]7943; Hash = '781024B1EBDA85E7BFDDB8610FB4E3817CEB1BFDBDE4BBF7CC6B8D6A9E0FBFA7' },
...
function Invoke-ParserSelfTest {
  ...
  foreach ($fixture in $rejected) {
    ...
    if (!$rejectedAsExpected) { throw 'The generation-result list parser accepted an ambiguous regression fixture.' }
  }
}
...
function Assert-ProcessBoundary($processes, [string]$phase) {
  if (@($processes | Where-Object { $_.ExecutablePath -eq $exe }).Count -ne 0) { throw "The new generation-result package is running during $phase; stop without process control." }
...
if ($SelfTestOnly) {
  [pscustomobject]@{
    Result = 'FND-09 generation-result package/list guard parser, no-force, color and stderr self-test passed.'
...
if ($verify.verified -ne $true -or $verify.packagedUtilityGenerationResult.isolatedMainHook -ne $true -or $verify.packagedUtilityGenerationResult.providerFreeLocalFixture -ne $true -or $verify.packagedUtilityGenerationResult.requestDerivedGenerationGate -ne $true -or $verify.packagedUtilityGenerationResult.queuedFreshWorkerRecovery -ne $true -or $verify.packagedUtilityGenerationResult.preloadPrivate -ne $true -or $verify.packagedUtilityGenerationResult.rendererPrivate -ne $true) {
  throw 'Static generation-result package markers failed.'
}
...
$observerResult = Invoke-NativeCapture { & $node '.\scripts\packaged-utility-generation-result.mjs' '.\tests\e2e\utility-containment.spec.ts' }
if ($observerResult.ExitCode -ne 0) { throw "External generation-result observer audit failed.`n$($observerResult.Text)" }
$observer = ConvertFrom-CapturedJson $observerResult 'External generation-result observer audit'
if ($observer.observerSha256 -ne $expectedObserverSha256 -or $observer.contract.onePackageLaunch -ne $true -or $observer.contract.playwrightWatchdogDisabled -ne $true -or $observer.contract.gracefulTimeoutRejectsWithoutControl -ne $true -or $observer.contract.toolDiscoveryPrivate -ne $true -or $observer.contract.invalidPayloadPrivate -ne $true -or $observer.contract.providerFree -ne $true -or $observer.contract.zeroInputOrOutputTargets -ne $true) {
  throw 'External observer contract returned contradictory no-force/privacy/provider/filesystem evidence.'
}
...
$env:AIDRAW_E2E_FND09_GENERATION_RESULT_EXE_SHA256 = $exeSha256
$env:AIDRAW_E2E_FND09_GENERATION_RESULT_ASAR_SHA256 = $asarSha256
...
[pscustomobject]@{
  Result = 'FND-09 generation-result package/static/list checkpoint passed without native application launch.'
  ...
  Selector = 'FND-09-GENERATION-RESULT'
