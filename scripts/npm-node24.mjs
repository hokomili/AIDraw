...
function nodeMajor(executable) {
  ...
  return Number(result.stdout.trim().replace(/^v/, '').split('.')[0]);
}
...
if (!node24) {
  throw new Error('Node 24.x was not found. Activate .nvmrc or set AIDRAW_NODE24_EXE to an absolute Node 24 executable.');
}
...
if (!npmCli) throw new Error('npm-cli.js was not found. Set AIDRAW_NPM_CLI to its absolute path.');
