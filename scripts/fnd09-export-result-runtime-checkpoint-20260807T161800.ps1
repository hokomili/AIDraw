...
$profile = 'E:\AIDraw\test-results\retained\aidraw-e2e-fnd09-export-result-native-20260807T161800'
$output = 'E:\AIDraw\test-results\playwright-fnd09-export-result-native-20260807T161800'
$connectionPath = Join-Path $profile 'mcp-connection.json'
$probePath = Join-Path $profile 'fnd09-export-result-probe.json'
$evidencePath = Join-Path $profile 'fnd09-export-result-evidence.json'
...
$sentinels = @(
  ...
  (Join-Path $profile 'fnd09-export-result-forbidden-network.json')
)
$packageListProfile = 'E:\AIDraw\test-results\retained\aidraw-e2e-fnd09-export-result-20260807T154601'
$packageListOutput = 'E:\AIDraw\test-results\playwright-fnd09-export-result-20260807T154601'
$agt14Root = 'E:\AIDraw\test-results\retained\aidraw-agt14-opencode-discovery-20260806T040500'
$packageGuard = 'E:\AIDraw\scripts\fnd09-export-result-package-checkpoint-20260807T154601.ps1'
...
$expectedExeSha256 = '2F74578467C53DDDFB43F13BB69B53F9B10E4C7F96E3A8E53C09413B7848CEA2'
$expectedAsarBytes = [long]47643512
$expectedAsarSha256 = 'DDE43A0F54647D56B78BADFDBCDF3BF80A0712F3CEF3A83700AE3C30E32E562D'
...
$fixedFiles = @(
  ...
  [pscustomobject]@{ Path = (Join-Path $repo 'scripts\packaged-utility-export-result.mjs'); Bytes = [long]7443; Hash = 'C101076DD5860BD2093C34E2DF34BE0CE6DB2039ACABA8D515E113A35AB79F6B' },
  ...
  [pscustomobject]@{ Path = $packageGuard; Bytes = [long]23813; Hash = '964437092975BC7EFA0F2CE1A7866EF8647CBB6D9A386337B965783980398B46' },
...
function Assert-ExactPlaywrightRunOutput([string]$Text) {
  ...
  if ([regex]::Matches($Text, $selectionPattern).Count -ne 1) { throw 'Runtime output must contain exactly one exact export-result test selection.' }
  if ([regex]::Matches($Text, $escapedTitle).Count -ne 1) { throw 'Runtime output repeats or omits the exact export-result title.' }
...
function Assert-ProcessBoundary($processes, [string]$phase) {
  ...
  if ($runOwned.Count -ne 0) {
    ...
    throw "Exact export-result package process exists during $phase (PIDs $survivors); retain every run artifact and do not control it."
  }
...
function Assert-SanitizedRetainedText([string]$Label, [string]$Text) {
  if ($Text -match '(?i)Bearer|Authorization|"token"\s*:|"data(?:Base64)?"\s*:') { throw "$Label contains credential or private artifact payload material." }
}
...
if ($SelfTestOnly) {
  [pscustomobject]@{
    Result = 'FND-09 export-result runtime guard parser/no-force/color/stderr self-test passed.'
...
if (!(Test-Path -LiteralPath $packageRoot -PathType Container)) { throw 'The immutable export-result package root is absent.' }
...
$env:AIDRAW_E2E_FND09_EXPORT_RESULT_EXE_SHA256 = $expectedExeSha256
$env:AIDRAW_E2E_FND09_EXPORT_RESULT_ASAR_SHA256 = $expectedAsarSha256
...
if ($verifyResult.ExitCode -ne 0) { throw "Static verification rejected the immutable export-result package.`n$($verifyResult.Text)" }
...
$observerAuditResult = Invoke-NativeCapture { & $node '.\scripts\packaged-utility-export-result.mjs' '.\tests\e2e\utility-containment.spec.ts' }
if ($observerAuditResult.ExitCode -ne 0) { throw "External export-result observer no-force audit failed.`n$($observerAuditResult.Text)" }
$observerAudit = ConvertFrom-CapturedJson $observerAuditResult 'External export-result observer audit'
if ($observerAudit.observerSha256 -ne $expectedObserverSha256 -or $observerAudit.contract.onePackageLaunch -ne $true -or $observerAudit.contract.playwrightWatchdogDisabled -ne $true -or $observerAudit.contract.gracefulTimeoutRejectsWithoutControl -ne $true -or $observerAudit.contract.toolDiscoveryPrivate -ne $true -or $observerAudit.contract.invalidPayloadPrivate -ne $true -or $observerAudit.contract.zeroExportTargets -ne $true) { throw 'External observer contract returned contradictory no-force/privacy/filesystem evidence.' }
...
$runResult = Invoke-NativeCapture { & $node '.\scripts\npm-node24.mjs' run test:e2e:only -- --grep=FND-09-EXPORT-RESULT --output=$output --workers=1 --reporter=list }
...
if ($probe.version -ne 1 -or $probe.scenario -ne 'FND-09 packaged export artifact result containment' -or $probe.result -ne 'passed') { throw 'Export-result probe identity/result failed.' }
...
if ($probe.realExporter.format -ne 'sprite-sheet' -or $probe.realExporter.primary -ne 'image/png' -or $probe.realExporter.companion -ne 'application/json') { throw 'The real exporter baseline evidence failed.' }
if ($probe.primary.error.message -ne 'Raster utility returned a malformed export artifact.' -or $probe.primary.resultReturnedToCaller -ne $false -or $probe.primary.payloadRetained -ne $false -or $probe.primary.rejectedBeforeCallerBase64Decode -ne $true -or $probe.primary.exportTargetWritten -ne $false -or [long]$probe.primary.workerPid -le 0) { throw 'Malformed-primary rejection/privacy evidence failed.' }
if ($probe.companion.error.message -ne 'Raster utility returned a malformed export companion.' -or $probe.companion.resultReturnedToCaller -ne $false -or $probe.companion.payloadRetained -ne $false -or $probe.companion.rejectedBeforeCallerBase64Decode -ne $true -or $probe.companion.exportTargetWritten -ne $false -or [long]$probe.companion.workerPid -le 0) { throw 'Malformed-companion rejection/privacy evidence failed.' }
foreach ($recovery in @($probe.primary.queuedRecovery, $probe.companion.queuedRecovery)) {
  ...
  if ([long]$recovery.primary.bytes -le 0 -or $recovery.primary.sha256 -cnotmatch '^[A-F0-9]{64}$' -or $recovery.primary.mimeType -ne 'image/png' -or $recovery.primary.extension -ne 'png') { throw 'Recovered PNG primary evidence failed.' }
  if ([long]$recovery.companion.bytes -le 0 -or $recovery.companion.sha256 -cnotmatch '^[A-F0-9]{64}$' -or $recovery.companion.mimeType -ne 'application/json' -or $recovery.companion.extension -ne 'json') { throw 'Recovered JSON companion evidence failed.' }
  if (@($recovery.warnings).Count -ne 0 -or @($recovery.rasterized).Count -ne 0) { throw 'Recovered export reports are not exact and empty.' }
}
...
if ($probe.primary.workerPid -eq $probe.primary.queuedRecovery.workerPid -or $probe.companion.workerPid -ne $probe.primary.queuedRecovery.workerPid -or $probe.companion.workerPid -eq $probe.companion.queuedRecovery.workerPid) { throw 'Worker replacement sequence is contradictory.' }
if (@(@($probe.primary.workerPid, $probe.companion.workerPid, $probe.companion.queuedRecovery.workerPid) | Select-Object -Unique).Count -ne 3) { throw 'The exact invalid/recovery worker sequence is not distinct.' }
if ($probe.primary.queuedRecovery.primary.sha256 -ne $probe.companion.queuedRecovery.primary.sha256 -or $probe.primary.queuedRecovery.companion.sha256 -ne $probe.companion.queuedRecovery.companion.sha256) { throw 'Recovered valid export hashes disagree.' }
...
if ($probe.filesystem.exportTargetsCreated -ne 0) { throw 'The probe reports an export target write.' }
...
if ($evidence.actor.name -ne 'FND-09 export result observer' -or $evidence.actor.color -ne '#3b728f') { throw 'Authenticated actor evidence failed.' }
if ($evidence.exportResult.result -ne 'passed' -or $evidence.exportResult.canonical.before.sha256 -ne $probe.canonical.before.sha256 -or $evidence.exportResult.primary.workerPid -ne $probe.primary.workerPid -or $evidence.exportResult.primary.queuedRecovery.workerPid -ne $probe.primary.queuedRecovery.workerPid -or $evidence.exportResult.companion.workerPid -ne $probe.companion.workerPid -or $evidence.exportResult.companion.queuedRecovery.workerPid -ne $probe.companion.queuedRecovery.workerPid) { throw 'Sanitized evidence does not embed the exact probe result.' }
...
if ($evidence.filesystem.exportTargetsCreated -ne 0) { throw 'Sanitized evidence reports an export target write.' }
...
if ($testExit -ne 0) { throw "Exact export-result test failed with exit code $testExit; retain every artifact and do not control any process.`n$runText" }
$runSelection = Assert-ExactPlaywrightRunOutput $runText

[pscustomobject]@{
  Result = 'FND-09 export-result exact packaged runtime acceptance passed.'
  ...
  Profile = $profile
  Output = $output
  PrimaryInvalidWorkerPid = $probe.primary.workerPid
  FirstRecoveryWorkerPid = $probe.primary.queuedRecovery.workerPid
  CompanionInvalidWorkerPid = $probe.companion.workerPid
  SecondRecoveryWorkerPid = $probe.companion.queuedRecovery.workerPid
  CanonicalUnchanged = $probe.canonical.unchanged
  GenerationLaneUntouched = $probe.generationLaneUntouched
  ExportTargetsCreated = $evidence.filesystem.exportTargetsCreated
