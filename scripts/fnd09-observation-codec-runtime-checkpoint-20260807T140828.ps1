...
$profile = 'E:\AIDraw\test-results\retained\aidraw-e2e-fnd09-observation-codec-colorfix-20260807T140828'
$output = 'E:\AIDraw\test-results\playwright-fnd09-observation-codec-colorfix-20260807T140828'
...
$failedRuntimeProfile = 'E:\AIDraw\test-results\retained\aidraw-e2e-fnd09-observation-codec-native-20260807T134502'
$failedRuntimeOutput = 'E:\AIDraw\test-results\playwright-fnd09-observation-codec-native-20260807T134502'
...
$failedRuntimeGuard = 'E:\AIDraw\scripts\fnd09-observation-codec-runtime-checkpoint-20260807T134502.ps1'
...
$expectedFailedProfileDirectories = 18
$expectedFailedProfileFiles = 26
$expectedFailedProfileBytes = [long]90015
$expectedFailedProfileManifestSha256 = 'D88AC6EB1497F51FD5C820C9B8E7790D1A57280671BE006236429AC8326D9E90'
$expectedFailedOutputDirectories = 0
$expectedFailedOutputFiles = 1
$expectedFailedOutputBytes = [long]45
$expectedFailedOutputManifestSha256 = 'E9C8773F9C896351AEBEBCF13A1A6958D8D3AC1A65EF1053D885244EBAA3B22C'
...
$fixedFiles = @(
  ...
  [pscustomobject]@{ Path = $failedRuntimeGuard; Bytes = [long]27645; Hash = '23293A92F6AFB6857A3F322F9860718EB25DEE3A3459CF6F5FF9CE428A7EDF6F' },
...
function Set-DeterministicColorEnvironment {
  ...
  if ([Environment]::GetEnvironmentVariable('FORCE_COLOR', 'Process') -ne $null) {
    throw 'FORCE_COLOR must be absent from the guarded process environment.'
  }
  if ([Environment]::GetEnvironmentVariable('NO_COLOR', 'Process') -ne '1') {
    throw 'NO_COLOR must be fixed to 1 in the guarded process environment.'
  }
}
...
function Invoke-NativeCapture([scriptblock]$Command) {
  ...
  try {
    # Windows PowerShell materializes native stderr as ErrorRecord objects. Capture
    # those records while treating the native exit code as the authoritative result.
...
function Invoke-EnvironmentSelfTest {
  ...
  try {
    ...
    $ok = Invoke-NativeCapture {
      & $node -e "process.stderr.write('benign-color-warning\n'); process.stdout.write('ok\n')"
    }
    if ($ok.ExitCode -ne 0 -or $ok.Text -notmatch 'benign-color-warning' -or $ok.Text -notmatch '(?m)^ok$') {
      throw 'Benign native stderr capture self-test failed.'
    }
    $failed = Invoke-NativeCapture {
      & $node -e "process.stderr.write('expected-failure\n'); process.exit(7)"
    }
    if ($failed.ExitCode -ne 7 -or $failed.Text -notmatch 'expected-failure') {
      throw 'Native nonzero-exit capture self-test failed.'
    }
    return [pscustomobject]@{
      ForceColorRemoved = $true
      NoColorFixed = $true
      BenignStderrExitCode = $ok.ExitCode
      NonzeroExitPreserved = $failed.ExitCode
    }
  } finally {
...
function Assert-TreeIdentity(
  ...
  if ($identity.Directories -ne $ExpectedDirectories -or
      $identity.Files -ne $ExpectedFiles -or
      $identity.Bytes -ne $ExpectedBytes -or
      $identity.Sha256 -ne $ExpectedSha256) {
    throw "Immutable failed runtime root changed during $Phase`: $Root"
  }
  return $identity
}
...
if ($SelfTestOnly) {
  [pscustomobject]@{
    Result = 'FND-09 observation-codec runtime guard parser/no-force/color/stderr self-test passed.'
    Ast = $guardAudit
...
$failedProfileBefore = Assert-TreeIdentity $failedRuntimeProfile $expectedFailedProfileDirectories $expectedFailedProfileFiles $expectedFailedProfileBytes $expectedFailedProfileManifestSha256 'preflight'
$failedOutputBefore = Assert-TreeIdentity $failedRuntimeOutput $expectedFailedOutputDirectories $expectedFailedOutputFiles $expectedFailedOutputBytes $expectedFailedOutputManifestSha256 'preflight'
...
if ($verifyResult.ExitCode -ne 0) { throw "Static verification rejected the immutable observation-codec package.`n$($verifyResult.Text)" }
...
$observerAuditResult = Invoke-NativeCapture { & $node '.\scripts\packaged-utility-observation-codec.mjs' '.\tests\e2e\utility-containment.spec.ts' }
if ($observerAuditResult.ExitCode -ne 0) { throw "External observation-codec observer no-force audit failed.`n$($observerAuditResult.Text)" }
$observerAudit = ConvertFrom-CapturedJson $observerAuditResult 'External observation-codec observer audit'
...
$listResult = Invoke-NativeCapture { & $node '.\scripts\npm-node24.mjs' run test:e2e:only -- --list --grep=FND-09-OBSERVATION-CODEC --output=$output --workers=1 --reporter=list }
$listExit = $listResult.ExitCode
...
$runResult = Invoke-NativeCapture { & $node '.\scripts\npm-node24.mjs' run test:e2e:only -- --grep=FND-09-OBSERVATION-CODEC --output=$output --workers=1 --reporter=list }
...
$failedProfileAfter = Assert-TreeIdentity $failedRuntimeProfile $expectedFailedProfileDirectories $expectedFailedProfileFiles $expectedFailedProfileBytes $expectedFailedProfileManifestSha256 'postflight'
$failedOutputAfter = Assert-TreeIdentity $failedRuntimeOutput $expectedFailedOutputDirectories $expectedFailedOutputFiles $expectedFailedOutputBytes $expectedFailedOutputManifestSha256 'postflight'
...
[pscustomobject]@{
  ...
  FailedRuntimeProfileManifestSha256 = $failedProfileAfter.Sha256
  FailedRuntimeOutputManifestSha256 = $failedOutputAfter.Sha256
