...
The human tileset inspector now edits an existing finite-orthogonal image collection through the same complete canonical `pixel.asset.replace` operation already available to the shared reducer boundary. Its UI adds no semantic MCP operation or filesystem authority: it selects exact sparse IDs, resolves each tile's own sprite dimensions, and changes only probability, typed properties, ordered animation, collision metadata, or the existing tileset drawing offset. Renderer checks handle observed topology/source drift, and optional read-only sprite dependency guards on that existing operation atomically refuse a queued-ahead source revision/dimension change at reducer admission without mutating a source. Source images, membership, local IDs, and collection creation/conversion remain outside both surfaces' bounded workflow.
