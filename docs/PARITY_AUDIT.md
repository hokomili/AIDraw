...
The human tileset inspector edits an existing finite-orthogonal image collection through the same complete canonical `pixel.asset.replace` operation already available to the shared reducer boundary. It selects exact sparse IDs, resolves each tile's own sprite dimensions, and changes probability, typed properties, ordered animation, collision metadata, or the existing tileset drawing offset. A later bounded lifecycle adds human creation from exact existing one-frame sprites plus append-above-span. Each chooser retains one immutable opening document/order/source observation and append target identity/revision; stable parent updates preserve selection, identity switches close the keyed panel, and displayed source/order/revision drift visibly refuses rather than reinterpreting the selection through newer props. Authenticated `pixel.image-collection.create|append` requests share the planner, stand alone under the observed document revision, and lower to existing asset add/replace operations rather than a new authority channel. Append on both surfaces carries exact tileset and old/new sprite revision/dimension guards, so queued-ahead source or topology change refuses atomically without mutating a source. Human creation uses canonical project order; semantic creation uses explicit request order. Source replacement/removal/reordering, external file picking, atlas conversion, and collection-backed tile objects/Wang/stamps remain outside both bounded surfaces.
