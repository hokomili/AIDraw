...
| Throughput | Pixel/tile RLE runs support one million cells, cross 32×32 chunks, exact inverse, progressive reveal/cancel, and sample-based backpressure. Numbered durable batches add progress, idempotent resume, cross-session token rebinding, and exact incarnation/transaction/request/phase ownership. Recovery restart retains the incarnation, while same-ID replacement retires old authority before dispatch; proven prepared-only abandonment reopens, dispatched cancellation waits for settlement, and ownerless recovery advances only from the exact transaction/client/fingerprint trace. Unrelated or unreadable evidence fails ambiguous without replay, advisory observer failure cannot change the outcome, and removed ledger records lose their public job mirrors. |
