...
The human tileset inspector edits an existing finite-orthogonal image collection through the same complete canonical `pixel.asset.replace` operation already available to the shared reducer boundary. It selects exact sparse IDs, resolves each tile's own sprite dimensions, and changes probability, typed properties, ordered animation, collision metadata, or the existing tileset drawing offset. Later bounded lifecycle work adds human creation from exact existing one-frame sprites, append-above-span, deliberate replacement of one exact local ID's source after a frozen document-wide impact summary, and removal of one exact record only after a frozen proof finds no retained direct map-cell, animation, tile-object, stamp, Wang, resolver-shadow, mode, resource, or stale-dependency uncertainty. Replacement changes no tile metadata/GID/source sprite and recomputes only the collection's nominal source envelope. Proven-unused removal leaves a sparse gap, first/raw GIDs, all other complete records, and every independently owned source sprite unchanged while recomputing the retained-source envelope. Each chooser retains one immutable opening document/order/source observation and exact target identity/revision; replacement/removal additionally bind tile/source identity and the impact/proof-owning document revision. Stable parent updates preserve selection, identity switches close the keyed panel, and displayed source/order/revision/impact/proof drift visibly refuses rather than reinterpreting the selection through newer props. Authenticated `pixel.image-collection.create|append|source.replace|source.remove` requests share the planners, stand alone under the observed document revision, and lower to existing asset add/replace operations rather than a new authority channel. Append/replacement/removal on both surfaces carry exact tileset and relevant sprite revision/dimension guards, so queued-ahead source or topology change refuses atomically without mutating a source. Human creation uses canonical project order; semantic creation uses explicit request order. A later finite-orthogonal tile-object checkpoint reuses the existing canonical object type and exact-ID planner for per-tile collection sprites, including animation-frame source selection, hit/culling, exact document/source-guarded human creation, document-bound semantic creation, and Tiled/native round trips. The bounded Wang checkpoint likewise admits exact sparse representatives plus guarded paint/erase through the shared planner and maintained Tiled/native paths. Source deletion/cascading cleanup, ID reassignment/reordering, external file picking, atlas conversion, Wang synthesis beyond exact authored signatures, collection-specific stamp topology, isometric/infinite collections, and packaged/native acceptance remain outside the cumulative surface.
