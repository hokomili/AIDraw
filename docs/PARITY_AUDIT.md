...
The human tileset inspector edits an existing finite-orthogonal image collection through the same complete canonical `pixel.asset.replace` operation already available to the shared reducer boundary. It selects exact sparse IDs, resolves each tile's own sprite dimensions, and changes probability, typed properties, ordered animation, collision metadata, or the existing tileset drawing offset. Later bounded lifecycle work adds human creation from exact existing one-frame sprites, append-above-span, and deliberate replacement of one exact local ID's source after a frozen document-wide impact summary and confirmation. The replacement changes no tile metadata/GID/source sprite and recomputes only the collection's nominal source envelope. Each chooser retains one immutable opening document/order/source observation and exact target identity/revision; replacement additionally binds the tile/old-source identity and impact-owning document revision. Stable parent updates preserve selection, identity switches close the keyed panel, and displayed source/order/revision/impact drift visibly refuses rather than reinterpreting the selection through newer props. Authenticated `pixel.image-collection.create|append|source.replace` requests share the planners, stand alone under the observed document revision, and lower to existing asset add/replace operations rather than a new authority channel. Append/replacement on both surfaces carry exact tileset and old/new sprite revision/dimension guards, so queued-ahead source or topology change refuses atomically without mutating a source. Human creation uses canonical project order; semantic creation uses explicit request order. Source removal/ID reassignment/reordering, external file picking, atlas conversion, and collection-backed tile objects/Wang/stamps remain outside both bounded surfaces.
