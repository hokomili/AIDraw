...
Both desktop CI and release workflows call that complete gate; `release:current` no longer substitutes a production-only audit. Package verification separately requires the exact packaged Electron version, confirms build-only dependencies are absent from the shipped ASAR, and scans every first-party JavaScript chunk in the ASAR—including main, preload, utility worker, importer, renderer, and dynamic build chunks—for orphaned/provider dependency roots plus known active generation, provider-credential, protected-storage, and retired workflow/chunk markers. On 2026-08-21, the exact current dirty subject received a clean lockfile-governed disposable install under Node 24.19.0 and produced a local `darwin/arm64` package. The verifier passed all 15 first-party JavaScript chunks, the exact 24/28/0/0/52 direct census, and 699-record external lock census for launcher SHA-256 `b0da1c2a929d5515b5ef781d1246211e3ad6a56659afda73882e4254012a9295` and ASAR SHA-256 `13ce119777e6bd17fec0ac2f09ec8e2c5368f1146fa09ee39a8e0596f772be45`. This is local Apple Silicon artifact evidence only, not Windows/Linux/Intel, hosted, signed-distribution, advisory-refresh, or independent-rebuild evidence.
