...
export function OnionSkinSettingsPanel({ settings, onChange, onClose }: OnionSkinSettingsPanelProps) {
  ...
  return <section id="onion-skin-settings" className="onion-settings-panel" aria-label="Onion skin settings">
    <header><span><strong>Onion skin</strong><small>Session preview · use Onion to show · document pixels do not change</small></span><button type="button" aria-label="Close onion skin settings" onClick={onClose}><X size={13} /></button></header>
    ...
    <footer><small>Farther frames fade as base opacity ÷ distance and render behind nearer frames.</small><button type="button" onClick={() => onChange({ ...DEFAULT_ONION_SKIN_SETTINGS })}><RotateCcw size={11} /> Reset</button></footer>
  </section>;
}
