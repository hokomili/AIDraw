...
async function writeConfig(configPath: string, contents: string, expectedExisting: string, now: Date): Promise<ConfigWriteResult> {
  ...
  if (existing !== expectedExisting) throw new Error(`${configPath} changed while AIDraw was preparing the update; retry setup.`);
  let backupPath: string | undefined;
  if (existed) {
    const stamp = now.toISOString().replace(/[:.]/g, '-');
    backupPath = `${configPath}.aidraw-backup-${stamp}`;
    await copyFile(configPath, backupPath);
    await chmod(backupPath, 0o600);
  }
  const temporaryPath = `${configPath}.aidraw-${process.pid}-${randomBytes(6).toString('hex')}.tmp`;
  try {
