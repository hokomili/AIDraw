...
export interface QuantizeImageOptions {
...
export function quantizeRgbaToPalette(
  ...
  if (rgba.length !== width * height * 4) throw new Error('RGBA data does not match the requested dimensions.');
  if (palette.length < 2 || palette.length > 256) throw new Error('Indexed conversion requires a palette with 2–256 entries.');
  const paletteLabs = palette.map((entry) => { const color = hexToRgba(entry.color); return { rgba: color, lab: oklab(color[0], color[1], color[2]) }; });
  const changes: Array<{ x: number; y: number; index: number }> = [];
  const errors = new Float32Array(width * height * 3);
  const bayer = [[0, 8, 2, 10], [12, 4, 14, 6], [3, 11, 1, 9], [15, 7, 13, 5]];
  for (let y = 0; y < height; y += 1) for (let x = 0; x < width; x += 1) {
    const offset = (y * width + x) * 4; const errorOffset = (y * width + x) * 3;
    const alpha = rgba[offset + 3] / 255;
    if (alpha < options.alphaThreshold) {
      if (options.includeTransparent) changes.push({ x, y, index: 0 });
      continue;
    }
    const threshold = options.dithering === 'bayer-4x4' ? (bayer[y % 4][x % 4] - 7.5) * 2 : 0;
    const red = Math.max(0, Math.min(255, rgba[offset] + errors[errorOffset] + threshold));
    const green = Math.max(0, Math.min(255, rgba[offset + 1] + errors[errorOffset + 1] + threshold));
    const blue = Math.max(0, Math.min(255, rgba[offset + 2] + errors[errorOffset + 2] + threshold));
    const lab = oklab(red, green, blue); let best = 1; let distance = Number.POSITIVE_INFINITY;
    for (let index = 1; index < paletteLabs.length; index += 1) {
      const candidate = paletteLabs[index]; if (candidate.rgba[3] === 0) continue;
    ...
    if (options.dithering === 'floyd-steinberg') {
      ...
      const diffuse = (targetX: number, targetY: number, weight: number) => {
        ...
        for (let channel = 0; channel < 3; channel += 1) errors[target + channel] += error[channel] * weight;
      };
...
export async function quantizeImageToPalette(
  encoded: Buffer,
  width: number,
  height: number,
  palette: PaletteEntry[],
  ...
  return quantizeRgbaToPalette(context.getImageData(0, 0, width, height).data, width, height, palette, options);
}
