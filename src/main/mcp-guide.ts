...
const helpTopics: Record<AIDrawHelpTopic, AIDrawHelpResult> = {
  quickstart: {
    ...
    steps: [
      ...
      'For open/save-as/import/export, retain the returned jobId. A human must approve in AIDraw; use job_manage action=wait or inspect, never attempt approval.',
    ],
...
export const AIDRAW_SERVER_INSTRUCTIONS = [
  ...
  'File calls may return owner-scoped jobs. A human alone approves in AIDraw; follow returned next guidance with job_manage wait|inspect.',
...
5. If a file call returns jobId, follow its next field with owner-scoped job_manage. Only a human can approve in AIDraw.
