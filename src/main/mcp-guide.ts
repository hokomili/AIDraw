...
Canonical operation families include document rename; asset/provenance; illustration artboard/layer/object/paint/guide/snap/animation; and pixel palette/frame/asset/cel/tilemap/link/stamp/font/cycle operations. Semantic helpers cover pixel fill/color/dither/text/selection/quantize/frame/stamp/link/image-collection/map/collision/Wang/variant workflows and illustration alignment/path/material/gradient/text/crop/filter/mask/animation workflows. pixel.image-collection.create, pixel.image-collection.append, pixel.image-collection.source.replace, and pixel.image-collection.source.remove lower bounded image-collection lifecycle intent into the existing asset add/replace protocol; each is one transaction guarded by the observed document revision, while append/replacement/removal also check the exact tileset plus every relevant source revision/dimension. Removal additionally proves the exact local ID has no retained map-cell, tile-object, animation, tile-stamp, or unsupported Wang reference, keeps the source sprite and every raw GID untouched, and refuses the final tile. pixel.tile-object.create shares the human exact-ID planner, including finite-orthogonal collection source resolution and document/map/tileset/source guards, and lowers to one complete map replacement. pixel.wang-terrain.stroke addresses one exact map/layer/attached-tileset/set/color, paint or erase, 1–65,536 ordered signed points, and the observed layer revision. For an image collection it is a sole document-revision-bound request, validates every sparse representative/mapping and source under production resolution, and retains finite-orthogonal mode. It applies the complete shared Wang plan only by lowering it to one canonical pixel.tilemap.set; unmatched and already-matching requests create no revision, while weighted variants use an intent-derived deterministic stream without reading or changing a document seed. Wang metadata operations use the same collection document/source guard without adding a reducer kind. Call aidraw_help with topic=operations for compact examples; tools still validate every canonical field, limit, graph reference, and revision.
