...
const helpTopics: Record<AIDrawHelpTopic, AIDrawHelpResult> = {
  ...
  operations: {
    ...
    steps: [
      ...
      'This topic returns operationKinds for every public canonical and semantic operation. Call aidraw_help with topic=operations and operation=<exact kind> for the payload JSON Schema and runtime preconditions, then construct canvas_apply.operations from that schema and observed state. JSON Schema does not replace graph, revision, or resource checks.',
      'Gradient endpoints are local object pixels. For radial gradients x1/y1 is the center and x2/y2 a radius point; zero radius uses the final stop color. Boolean edits commit only a nonempty supported single-subpath result; compound results are refused without changing operands. Populated object groups must stay in their existing vector layer.',
...
## Operation-specific discovery

Call \`aidraw_help\` with \`topic=operations\` to list \`operationKinds\`. Add \`operation\` with one exact kind to obtain its complete input JSON Schema and runtime preconditions, using the same validators as canonical or semantic admission. For example, \`{"topic":"operations","operation":"illustration.gradient.set"}\` explains required geometry, stops and revision. Construct edits from these results plus \`canvas_observe\`; do not guess hidden fields. Unknown or retired kinds are refused.

Gradient coordinates are local object pixels, before object/group transforms. Radial endpoints describe a center and radius point, not two circle centers; a zero radial radius uses the final stop color. Boolean modes require a nonempty result within the single-simple-subpath editing envelope. Holes or disjoint outlines may make a result unsupported even for valid operands, and rejection leaves the operands intact. A populated group can move only within its current vector layer; mask/reference restrictions also apply. Capability refusals are not resolved by retrying unchanged intent.
