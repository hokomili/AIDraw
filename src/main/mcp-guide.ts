...
const helpTopics: Record<AIDrawHelpTopic, AIDrawHelpResult> = {
  ...
  operations: {
    ...
    steps: [
      ...
      'Pixel semantic operations include flood-fill, replace-color, palette replace-delete, adjust-index, ordered-dither, bitmap-text.paint, selection.transform, image.quantize, frame helpers, stamps, project links, bounded image-collection create/append, map objects, collisions, pixel.wang-terrain.stroke, Wang metadata, and tile variants.',
      'pixel.image-collection.create references 1–1,023 exact one-frame sprite IDs in request order; pixel.image-collection.append adds one exact sprite at max local ID + 1. Each must be the only request in its transaction and carries the observed document revision; append also carries the observed tileset revision.',
...
Canonical operation families include document rename; asset/provenance; illustration artboard/layer/object/paint/guide/snap/animation; and pixel palette/frame/asset/cel/tilemap/link/stamp/font/cycle operations. Semantic helpers cover pixel fill/color/dither/text/selection/quantize/frame/stamp/link/image-collection/map/collision/Wang/variant workflows and illustration alignment/path/material/gradient/text/crop/filter/mask/animation workflows. pixel.image-collection.create and pixel.image-collection.append lower exact one-frame sprite identities into the existing asset add/replace protocol; each is one transaction guarded by the observed document revision, and append also checks the exact tileset plus every source revision/dimension. pixel.wang-terrain.stroke addresses one exact map/layer/attached-tileset/set/color, paint or erase, 1–65,536 ordered signed points, and the observed layer revision. It applies the complete shared Wang plan only by lowering it to one canonical pixel.tilemap.set; unmatched and already-matching requests create no revision, while weighted variants use an intent-derived deterministic stream without reading or changing a document seed. Call aidraw_help with topic=operations for compact examples; tools still validate every canonical field, limit, graph reference, and revision.
